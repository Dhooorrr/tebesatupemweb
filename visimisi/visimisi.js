function toggleMenu() {
    ...
    function slide(direction) {
      ...
      const cardWidth = kartu[0].getBoundingClientRect().width + 20;
      const maxSlide = total - visible;

      slidePos += direction;
      if (slidePos < 0) slidePos = 0;
      if (slidePos > maxSlide) slidePos = maxSlide;

      track.style.transform = `translateX(-${slidePos * cardWidth}px)`;
    }

    btnKiri.onclick = () => slide(-1);
    btnKanan.onclick = () => slide(1);
